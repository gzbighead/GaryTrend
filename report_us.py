...
def build_html(report_date, signal_dates, signals_by_date,
    ...
    ai_block = f"""<div style="background:#f8f9ff;border-left:4px solid #1a56db;border-radius:4px;padding:14px;margin-bottom:20px;">
      <div style="font-size:13px;font-weight:bold;color:#1a56db;margin-bottom:8px;">🤖 美股市场分析</div>
    ...
    <table width="600" cellpadding="0" cellspacing="0" border="0" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#222;padding:16px;">
      <tr><td>
        <h2 style="margin:0 0 4px;font-size:17px;">📈 美股市场趋势报告</h2>
...
def send_email(html, report_date):
    ...
    res = requests.post(
        ...
        json={
            "from":    EMAIL_FROM,
            "to":      EMAIL_TO,
            "subject": f"投资分析 - 美股趋势报告 {report_date}",
            "html":    html,
        },
    )
...
def main():
    ...
    print(f"[开始] 美股报告 {report_date} 共{len(WATCHLIST_US)}只")

    # 1. 扫描所有标的
    results = []
    for symbol, name in WATCHLIST_US:
        try:
            r = scan_symbol(symbol, name, classify_us, SIGNAL_DAYS, TREND_DAYS)
            if r is None:
